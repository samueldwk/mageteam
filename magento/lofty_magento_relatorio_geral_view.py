...
for client in c_list:
    ...
    try:
        ...
        if billing_response.data:
            print(
                f"✅ Successfully fetched {len(billing_response.data)} billing records!"
            )
        else:
            print("⚠️ No billing data found.")
        ...
        if returns_response.data:
            print(
                f"✅ Successfully fetched {len(returns_response.data)} return records!"
            )
        else:
            print("⚠️ No returns data found.")
        ...
        if billing_response.data or returns_response.data:
            ...
            billing_by_date = (
                {
                    record["date"]: record["total_invoicing"]
                    for record in billing_response.data
                }
                if billing_response.data
                else {}
            )
            returns_by_date = (
                {
                    record["date"]: record["total_value"]
                    for record in returns_response.data
                }
                if returns_response.data
                else {}
            )
            ...
            percentage_by_date = {}
            for date in set(billing_by_date.keys()) | set(
                returns_by_date.keys()
            ):
