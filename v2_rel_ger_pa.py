...
# d1 = datetime.datetime(2024, 10, 19).date()
...
c_list_sku_8 = {
    "Cliente": ["dadri", "othergirls", "nobu", "talgui"],
    "Index": 8,
}
c_list_sku_9 = {"Cliente": ["ajobrand", "french", "una"], "Index": 9}
c_list_sku_20 = {"Cliente": ["paconcept"], "Index": 20}
...
c_list_sku = [c_list_sku_0, c_list_sku_8, c_list_sku_9, c_list_sku_20]
...
for cliente in c_list:
    ...
    for c_sku in c_list_sku:
        if cliente in c_sku["Cliente"]:
            index = c_sku["Index"]
            if index == 2:
                df_estoque["Cod. Modelo + Cor"] = df_estoque["Código"].str[:-2]
            else:
    ...
    if cliente == "paconcept":
        estoque_preco_df = estoque_preco_df[
            ~estoque_preco_df.apply(
                lambda row: row.astype(str).str.contains("CE").any(), axis=1
            )
        ]
    ...
    if cliente == "paconcept":
        estoque_preco_df = estoque_preco_df[
            ~estoque_preco_df.apply(
                lambda row: row.astype(str).str.contains("CH").any(), axis=1
            )
        ]
    ...
    if cliente == "paconcept":
        estoque_preco_df = estoque_preco_df[
            ~estoque_preco_df.apply(
                lambda row: row.astype(str).str.contains("VI").any(), axis=1
            )
        ]
    ...
    if cliente == "paconcept":
        estoque_preco_df = estoque_preco_df[
            ~estoque_preco_df.apply(
                lambda row: row.astype(str).str.contains("PR").any(), axis=1
            )
        ]
    ...
    if cliente == "paconcept":
        estoque_preco_df = estoque_preco_df[
            ~estoque_preco_df.apply(
                lambda row: row.astype(str).str.contains("TESTE").any(), axis=1
            )
        ]
    ...
    for c_sku in c_list_sku:
        if cliente in c_sku["Cliente"]:
            index = c_sku["Index"]
            if index == 2:
                df_vendas["Cod. Modelo + Cor"] = df_vendas["Codigo SKU"].str[
                    :-2
                ]
            else:
