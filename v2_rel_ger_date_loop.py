...
data_inicio = datetime.date(2024, 1, 13)
...
for dates in range(date_range):
    ...
    for cliente in c_list:
        ...
        if cliente == "una":
            estoque_preco_df = estoque_preco_df[
                ~estoque_preco_df.apply(
                    lambda row: row.astype(str).str.contains("VP").any(),
                    axis=1,
                )
            ]
        ...
        estoque_preco_total_df = pd.DataFrame(
            {
                ...
                "Poder de Venda Total": [
                    estoque_preco_df["Poder de Venda"].sum()
                ],
        ...
        df_vendas["CMV"] = (
            df_vendas["Preco de Custo"] * df_vendas["Quantidade"]
        )
        ...
        # # Use groupby to sum 'Poder de Venda' for each 'Desconto Group'
        result1 = (
            df_vendas_prod_situacao.groupby(
                "Faixa de Desconto", observed=False
            )["Valor de Venda Produto Total"]
            .sum()
            .reset_index()
        )
        ...
        venda_desconto_final = venda_desconto_final.iloc[1:].reset_index(
            drop=True
        )
