...
dic_nomes = {'ajobrand': 'aJo Brand', 'dadri': 'Dadri', 'french': 'French', 'infini': 'Infini', 'mun': 'Mun', 'othergirls': 'Other Girls', 'talgui': 'Talgui', 'una': 'Una'}
...
report_sku_error_all = {'Cliente': [], 'Data': [], 'Cod. Modelo + Cor': []}
...
for cliente in c_list:
    ...
    estoque_preco_df = pd.merge(
        df_estoque_grouped, df_produtos, on="Cod. Modelo + Cor", how="left"
    )
    
    ##### REPORT PRODUTO FILHO QUE NAO TEM PAI
    
    report_sku_error = estoque_preco_df[estoque_preco_df.isna().any(axis=1)]['Cod. Modelo + Cor'].values.tolist()
    report_sku_error_all['Cliente'].append(cliente) 
    report_sku_error_all['Cod. Modelo + Cor'].append(report_sku_error)
    report_sku_error_all['Data'].append(datatxt)
    ...
    if cliente == 'una':
        estoque_preco_df = estoque_preco_df[~estoque_preco_df.apply(lambda row: row.astype(str).str.contains('VP').any(), axis=1)]
    ...
    # Remove sales from 'Nome do contato' = 'LOJA JP'
    
    df_vendas.drop(df_vendas[df_vendas['Nome do contato'] == 'LOJA JP'].index, inplace=True)
...
#%% UPDATE GOOGLE SHEETS RELATÓRIO GERAL
    ...
    #%% UPDATE GOOGLE SHEETS REPORT DE PROBLEMAS DE SKU
    
    # gc = gspread.oauth()
    
    # sh = gc.open("Geral - Report Problema").worksheet('Página1')
    
    # df_report_sku_error = pd.DataFrame([report_sku_error_all]).apply(lambda x: x.explode() if x.dtype == 'O' and isinstance(x.iloc[0], list) else x)

    # sh.append_rows(df_report_sku_error.values.tolist(), table_range="A1")
