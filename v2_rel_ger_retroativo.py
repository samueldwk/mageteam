...
# d1 = datetime.datetime(2024, 10, 27).date()
...
for cliente in c_list:
    ...
    f_estoque_path = rf"{os.getenv('path_mage')}m&p\Relatorios Diarios\{dataname1}\{cliente}_Estoque_{dataname1}.xlsx"
    ...
    f_produtos_path = rf"{os.getenv('path_mage')}m&p\Relatorios Diarios\{dataname1}\{cliente}_Produtos_{dataname1}.xlsx"
    ...
    f_vendas_path = rf"{os.getenv('path_mage')}m&p\Relatorios Diarios\{dataname1}\{cliente}_Vendas_{dataname1}.csv"
    ...
    df_rel_ger = rel_ger[desired_order]
    ...
    print(f"{dic_nomes[cliente]} - Relatório Gerencial E-Commerce - {datatxt}")

    cell_d1 = sh.find(datatxt).row

    sh.update(range_name=f"A{cell_d1}:V{cell_d1}", values=[df_list_final[0]])

    # %% d2

    f_estoque_path = rf"{os.getenv('path_mage')}m&p\Relatorios Diarios\{dataname2}\{cliente}_Estoque_{dataname2}.xlsx"
    df_estoque = pd.read_excel(f_estoque_path, dtype={"Código": str})
    ...
    # IMPORT EXCEL PRODUTOS TO FIX AND ORGANIZE
    f_produtos_path = rf"{os.getenv('path_mage')}m&p\Relatorios Diarios\{dataname2}\{cliente}_Produtos_{dataname2}.xlsx"
    df_produtos = pd.read_excel(
        f_produtos_path, dtype={"Codigo SKU": str}
    ).rename(
        columns={
    ...
    f_vendas_path = rf"{os.getenv('path_mage')}m&p\Relatorios Diarios\{dataname2}\{cliente}_Vendas_{dataname2}.csv"
    df_vendas = pd.read_csv(
        f_vendas_path,
        sep=";",
        dtype={
    ...
    print(
        f"{dic_nomes[cliente]} - Relatório Gerencial E-Commerce - {datatxt2}"
    )

    cell_d1 = sh.find(datatxt2).row
    ...
    # IMPORT EXCEL PRODUTOS TO FIX AND ORGANIZE
    f_produtos_path = rf"{os.getenv('path_mage')}m&p\Relatorios Diarios\{dataname3}\{cliente}_Produtos_{dataname3}.xlsx"
    df_produtos = pd.read_excel(
        f_produtos_path, dtype={"Codigo SKU": str}
    ).rename(
        columns={
    ...
    f_vendas_path = rf"{os.getenv('path_mage')}m&p\Relatorios Diarios\{dataname3}\{cliente}_Vendas_{dataname3}.csv"
    df_vendas = pd.read_csv(
        f_vendas_path,
        sep=";",
        dtype={
    ...
    print(
        f"{dic_nomes[cliente]} - Relatório Gerencial E-Commerce - {datatxt3}"
    )

    cell_d1 = sh.find(datatxt3).row
